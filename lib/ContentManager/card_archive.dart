...
class CardArchive extends StatefulWidget{

  const CardArchive({super.key});

  @override
  State<StatefulWidget> createState() => _CardArchive();

}

class _CardArchive extends State<CardArchive>{
  ...
  List<PlayCard> calculate(int i){
    ...
    for(int x = 0; x < 9; x++) {
      ...
      if(transfer < InventoryManager.cardsOwned.length){
          cards.add(PlayCard(card: InventoryManager.cardsOwned[transfer], enableText: false, ignoreAmountRule: true,));
      }
      else{
        cards.add(const PlayCard(enableText: false, ignoreAmountRule: true,));
      }
    }
    return cards;
  }
